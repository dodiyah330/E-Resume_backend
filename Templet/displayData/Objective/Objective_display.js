        ...
        return (
            <>
                {this.Obj_layouts.map((item, index) => [
                    <div key={index}>
                        <SelectedLayoutfilter component={item} id={this.Obj_layouts.indexOf(item)} selectedLayout={this.props.selectedLayout} objective={() => this.props.branch_resume === null ? this.state.objective : this.props.branch_resume()} />
                    </div>
                ])}
            </>
        )
